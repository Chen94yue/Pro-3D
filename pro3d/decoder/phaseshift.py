...
class Phaseshift(nn.Module):
    ...
    def get_background(self, pimgs):
        sin_map = pimgs[0] * torch.sin(2*torch.pi*0/self.ps_step)
        for i in range(1, self.ps_step):
            sin_map += (pimgs[i] * torch.sin(2*torch.pi*i/self.ps_step))
        cos_map = pimgs[0] * torch.cos(2*torch.pi*0/self.ps_step)
        for i in range(1, self.ps_step):
            cos_map += (pimgs[i] * torch.cos(2*torch.pi*i/self.ps_step))
        bg = 2 / self.ps_step * torch.sqrt(sin_map**2 + cos_map**2)
        return bg

    def forward(self, pimgs):
        val1 = pimgs[0] * torch.sin(2*torch.pi*0/self.ps_step)
        for i in range(1, self.ps_step):
            val1 += (pimgs[i] * torch.sin(2*torch.pi*i/self.ps_step))

        val2 = pimgs[0] * torch.cos(2*torch.pi*0/self.ps_step)
        for i in range(1, self.ps_step):
