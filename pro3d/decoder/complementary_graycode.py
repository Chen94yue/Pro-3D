...
LastEditTime: 2022-10-24 15:40:24
...
class ComplementaryGraycode(nn.Module):
    ...
    def __init__(self,
        ...
        self.col_batch_index = torch.tensor(
            [i for i in range(0, self.num_of_gc_imgs)])
        self.col_batch_ratio = torch.tensor(
            [2**(i-1) for i in range(self.num_of_gc_imgs, 0, -1)]).view(-1, 1, 1)
        if not col_only:
            self.row_batch_index = torch.tensor(
                [i for i in range(self.num_of_gc_imgs, self.num_of_gc_pattern_imgs)])
            self.row_batch_ratio = torch.tensor(
                [2**(i-1) for i in range(self.num_of_gc_imgs, 0, -1)]).view(-1, 1, 1)
    ...
    def forward(self, images):
        ...
        col_bg = self.phaseshift_decoder.get_background(
            ps_images[:self.num_of_ps_imgs])
        if not self.col_only:
            row_bg = self.phaseshift_decoder.get_background(
                ps_images[self.num_of_ps_imgs:])
        else:
            row_bg = col_bg
        ...
        if not self.col_only:
            ...
            dec_row_v2 = torch.sum(gray_row_k2.int() *
                                   self.row_batch_ratio, dim=0)
