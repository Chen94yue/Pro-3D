...
LastEditTime: 2022-10-24 15:40:58
...
class StereoRig:
    def __init__(self, res1, res2, intrinsic1, intrinsic2, distCoeffs1, distCoeffs2, R, T, F=None, E=None, reprojectionError=None):
        ...
        self.distCoeffs1 = np.array(distCoeffs1) if distCoeffs1 is not None else np.zeros(
            5)  # Convert to numpy.ndarray
        self.distCoeffs2 = np.array(
            distCoeffs2) if distCoeffs2 is not None else np.zeros(5)
        self.R = np.array(R)
    ...
    def getBaseline(self):
        ...
        # No need to do C2 - C1 as C1 is always zero (origin of world system)
        return np.linalg.norm(C2)
...
class GraycodeCrossRebuilder(BaseRebuilder):
    ...
    def __init__(self, calibrate_param_file, proj_shape, downsample_ratio):
        ...
        self.R_inv = np.hstack(
            (np.vstack((self.R_inv, np.zeros((1, 3)))), np.zeros((4, 1))))
        self.R_inv[3, 3] = 1
    ...
    def rebuild(self, pp):
        ...
        pc = np.concatenate(
            (pc, np.ones((pc.shape[0], pc.shape[1], 1))), axis=2).astype(np.float32)
        # Get world points
        ...
        finalPoints = cv2.perspectiveTransform(
            pw, self.R_inv)  # .reshape(-1, 3)
