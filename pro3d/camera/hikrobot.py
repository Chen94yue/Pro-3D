...
class HikrobotCamera(BaseCamera):
    ...
    def __init__(self,
        ...
        for key, value in self.cam_serial_dict:
            self.cam[key] = (MvCamera(), cast(deviceList.pDeviceInfo[value], POINTER(MV_CC_DEVICE_INFO)).contents)
    ...
    def set_camera(self):
        ...
        stBool = c_bool(False)
        for i in self.cam.keys():
            ret = self.cam[i].MV_CC_GetBoolValue("AcquisitionFrameRateEnable", stBool)
            if ret != 0:
        ...
        for i in self.cam.keys():
            ret = self.cam[i].MV_CC_SetEnumValue("TriggerMode", MV_TRIGGER_MODE_OFF)
            if ret != 0:
        ...
        for i in self.cam.keys():
            ...
            memset(byref(self.stFrameInfo_dict[i]), 0, sizeof(self.stFrameInfo_dict[i]))

        for i in self.cam.keys():
            ret = self.cam[i].MV_CC_SetEnumValue("ExposureMode", MV_EXPOSURE_MODE_TIMED)
            if ret != 0:
                self.logger.info("Set exposure mode failed.")
                sys.exit()
        for i in self.cam.keys():
            ret = self.cam[i].MV_CC_SetEnumValue("ExposureAuto", MV_EXPOSURE_AUTO_MODE_OFF)
            if ret != 0:
                self.logger.info("Set exposure auto failed.")
                sys.exit()
        for i in self.cam.keys():
            ret = self.cam[i].MV_CC_SetFloatValue("ExposureTime",float(self.exposureTime)) # us
            if ret != 0:
    ...
    def get_image(self):
        self.images = {}
        for i in self.cam.keys():
            ret = self.cam[i].MV_CC_GetOneFrameTimeout(self.data_buf_dict[i], self.payload_size, self.stFrameInfo_dict[i], 1000)
            if ret == 0:
                frame_data = np.ctypeslib.as_array(self.data_buf_dict[i])
                if self.rgb:
                    self.images[i] = frame_data.reshape((self.stFrameInfo_dict[i].nHeight, self.stFrameInfo_dict[i].nWidth, 3))
                    self.images[i] = cv2.cvtColor(self.images[i], cv2.COLOR_RGB2BGR)
                else:
                    self.images[i] = frame_data.reshape((self.stFrameInfo_dict[i].nHeight, self.stFrameInfo_dict[i].nWidth))
            else:
